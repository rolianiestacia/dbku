...
$sql=mysqli_query($conn,"SELECT * FROM employee WHERE emp_id='$user'");
$row=mysqli_fetch_assoc($sql);
  ...
  if(isset($_POST['change-pw']))
  {
    ...
    if($curr_pw != $result['emp_password'])
    {
    ...
    if (count($errors) == 0){
        if($curr_pw==$result['emp_password'] && $new_pw == $confirm_new_pw)
        {
            $query = mysqli_query($conn,"UPDATE employee SET emp_password='$new_pw' WHERE emp_id='$user'");
            if($query)
            {
                 $_SESSION['login']="Your password is successfully updated! Please sign in again using the new password!";
                unset($_SESSION['user']);
                echo '<script>window.open("signin-employee.php","_parent");</script>';
                     ...
                     <div class="row justify-content-center">
                        <div class="col-lg-11">
                            <div class="card shadow-lg border-0 rounded-lg mt-6">
                                <div class="card-header"><h4 style="font-family: 'Lucida Console', 'Courier New', monospace;" class="text-center font-weight-light my-3"><i class="fas fa-key fa-sm fa-fw"></i> Update Profile</h4></div>
                                <div class="card-body">
                                    <form method="post" action="change-pw-employee.php">
